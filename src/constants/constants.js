export const projects = [
  {
    title: 'Waxwing',
    description:"React single-page app that allows users to view recipes for drinks, favourite and save recipes, as well as sharing recipes with friends. Users can also create their own recipe.",
    image: '/images/1.png',
    tags: ['React', 'PSQL', 'Knex.js', 'Axios', 'Material UI'],
    live: 'https://google.com',
    github: 'https://github.com/Winnie0313/Waxwing',
    id: 0,
  },
  {
    title: 'Wiki-Maps',
    description: "Web application that allows users to collaboratively create maps that list multiple locations. For example: 'Best Places to Eat Around Town' or 'Locations of Movie Scenes'.",
    image: '/images/2.png',
    tags: ['Express', 'Node', 'EJS', 'Ajax', 'jQuery'],
    live: 'https://google.com',
    github: 'https://github.com/Winnie0313/Wiki-Maps',
    id: 1,
  },
  {
    title: 'Jungle E-commerce',
    description: "A mini e-commerce application built with Ruby and Rails 6.1 that allows users to shop plants and admin to manage products.",
    image: '/images/3.png',
    tags: ['Ruby on Rails', 'Bootstrap', 'PostgreSQL'],
    live: 'https://google.com',
    github: 'https://github.com/Winnie0313/jungle-rails',
    id: 2,
  },
  {
    title: 'Portfolio Website',
    description: "Single-page website that displays my projects, skills and experience.",
    image: '/images/4.png',
    tags: ['React', 'Next.js', 'Styled Components'],
    live: 'https://google.com',
    github: 'https://github.com/Winnie0313/portfolio',
    id: 3,
  },
];

export const TimeLineData = [
  { year: "2022-Present", position: 'Software Development Intern at GAO Tek', text: 'Work with data structures, and analyze software for vulnerabilities. Research and write technical documentation.', image: '/images/GAOTek.png', },
  { year: 2022, position: 'Diploma on Web Development', text: 'at Lighthouse Lab', image: '/images/lighthouseLab.png', },
  { year: "2020-2022", position: 'High School Math Teacher', text: 'at Surrey School District and Phoenix Magnet Academy', image: '/images/surreySchools.png', },
  { year: 2017, position: 'Web Development Intern at LESSO', text: 'Developed and maintained the eCommerce platform for the company', image: '/images/lesso.png', },
  { year: "2014-2018", position: 'B.Sc. Combined Major in Math and Science at UBC', text: 'CS Courses: CPSC 221-Basic Algorithms and Data Structures, CPSC 210-Software Construction, CPSC 121-Models of Computation, CPSC 110-Computation, Programs, and Programming ', image: '/images/ubc.png', },
];